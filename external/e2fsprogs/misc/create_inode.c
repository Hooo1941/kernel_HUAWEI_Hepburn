...
#include <string.h>
static int is_no_sparse_file;
const char **no_sparse_file_type_list;
static int file_type_list_count;
#define USE_SPARSE_SAVE_FILE 0
#define NO_SPARSE_SAVE_FILE 1

/*
 * check filename whether is ext type file inputed or not
 */
static void check_sparse_filetype(const char *filename)
{
	const char *extfilename = NULL;
	int idex;
	size_t extfnlen;

	is_no_sparse_file = USE_SPARSE_SAVE_FILE;
	if (filename == NULL || file_type_list_count == 0 || no_sparse_file_type_list == NULL)
		return;
	...
	extfilename++;
	extfnlen = strlen(extfilename);
	if (extfnlen == 0)
		return;
	for (idex = 0; idex < file_type_list_count; idex++) {
		if (extfnlen != strlen(no_sparse_file_type_list[idex]))
			continue;

		if (strncmp(extfilename, no_sparse_file_type_list[idex], extfnlen) == 0) {
			is_no_sparse_file = NO_SPARSE_SAVE_FILE;
			break;
		}
	}
}
...
/*
 * get file save data type
 */
static int check_file_use_sparse(void)
{
	return is_no_sparse_file;
}

/*
 * set special file type list from the caller
 */
void set_no_sparse_file_list(const char **exttype, int typecount)
{
	if (typecount == 0 || exttype == NULL)
		return;

	no_sparse_file_type_list = exttype;
	file_type_list_count = typecount;
}
#endif
	...
	for (off = start; off < end; off += COPY_FILE_BUFLEN) {
		...
		for (bpos = 0, ptr = buf; bpos < got; bpos += fs->blocksize) {
			...
			if (blen > got - bpos)
				blen = got - bpos;
#ifdef HW_FILE_SAVE_NO_SPARSE
			if (check_file_use_sparse() == USE_SPARSE_SAVE_FILE) {
#endif
