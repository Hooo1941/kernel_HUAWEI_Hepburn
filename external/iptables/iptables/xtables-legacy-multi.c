...
int main(int argc, char **argv)
{
	...
	// ip6tables cmd is not printed to avoid excessive logs
	if (!strcmp(cmd, "iptables") || !strcmp(cmd, "iptables-restore") ||
	    !strcmp(cmd, "ip6tables-restore")) {
		ALOGI("cmd:%s, pid:%d, ppid:%d", cmd, getpid(), getppid());
	}

	ret = subcmd_main(argc, argv, multi_subcommands);
	ALOGI("iptable exit,ret = %d", ret);
	return ret;
}
