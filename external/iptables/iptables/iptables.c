	     ...
	     struct xtc_handle *handle)
{
	...
	if (count != 0) {
		ALOGI("append_entry, print v4 firewall, count:%d", count);
	}
	return ret;
}
	      ...
	      struct xtc_handle *handle)
{
	...
	if (verbose) {
		print_firewall_line(fw, handle);
		ALOGI("replace_entry, print v4 firewall");
	}
	     ...
	     struct xtc_handle *handle)
{
	...
	if (count != 0) {
		ALOGI("insert_entry, print v4 firewall, count:%d", count);
	}
	return ret;
}
	     ...
	     const struct xtables_target *target)
{
	...
	if (count != 0) {
		ALOGI("delete_entry, print v4 firewall, count:%d", count);
	}
	return ret;
}
	    ...
	    const struct xtables_target *target)
{
	...
	if (count != 0) {
		ALOGI("check_entry, print v4 firewall, count:%d", count);
	}
	return ret;
}
	     ...
	     int expanded, int linenumbers, struct xtc_handle *handle)
{
	...
	if (count != 0) {
		ALOGI("list_entries, print v4 firewall, count:%d", count);
	}
		...
		struct xtc_handle **handle, bool restore)
{
	...
	case CMD_APPEND:
		/* if specified rule already existed within mobile_reject_list , wifi_reject_list,
		   bw_penalty_box or bw_happy_box, no need to append. */
		if (!strcmp(chain, "mobile_reject_list") || !strcmp(chain, "wifi_reject_list") ||
			!strcmp(chain, "bw_penalty_box") || !strcmp(chain, "bw_happy_box")) {
	...
	case CMD_INSERT:
		/* if specified rule already existed within mobile_reject_list , wifi_reject_list,
		   bw_penalty_box or bw_happy_box, no need to insert. */
		if (!strcmp(chain, "mobile_reject_list") || !strcmp(chain, "wifi_reject_list") ||
			!strcmp(chain, "bw_penalty_box") || !strcmp(chain, "bw_happy_box")) {
			ret = check_entry(chain, e,
					   nsaddrs, saddrs, smasks,
