...
#ifdef BG_FSCK
const char FS_MGR_USERDATA_PARTITION_LNK[] = "/dev/block/by-name/userdata";
const int MAX_USERDATA_DEVICE_NAME_LEN = 32;
#endif

// For 'system', 'product' (optional), 'vendor' (mandatory) and/or 'odm' (optional).
// and/or 'odm' (optional).
#define MAX_FILE_CONTEXT_SIZE 5
#define FTS_FIRST_LEVEL_SUB_DIR 1
#define DATA_APP_EL1_DATA "/data/ohos_data/app/el1/100/database/"
#define DATA_APP_EL1_BASE "/data/ohos_data/app/el1/100/base/"
#define DATA_APP_EL2_DATA "/data/ohos_data/app/el2/100/database/"
#define DATA_APP_EL2_BASE "/data/ohos_data/app/el2/100/base/"
#define DATA_APP_EL3 "/data/ohos_data/app/el3/"
#define DATA_APP_EL4 "/data/ohos_data/app/el4/"
enum label_check_flag {
	LABEL_LOOKUP_FAIL,
	LABEL_GET_INODE_FAIL,
	LABEL_COMPARE_EQUAL,
	LABEL_COMPARE_NOT_EQUAL,
	LABEL_HASH_EQUAL,
	LABEL_HASH_NOT_EQUAL,
	LABEL_HASH_UPDATE_SUCCESS,
	LABEL_HASH_UPDATE_FAILED,
	LABEL_CHECK_SUCCESS,
	LABEL_CHECK_FAILED
};
...
#define DATA_LOG_FILEINDEX "/data/log/fileindex"
...
#ifdef BG_FSCK
static int RestoreconSbUserdata(const char *pathName, const struct stat *sb,
                                bool nochange, bool verbose)
{
    char *secontext = NULL;
    char *oldSecontext = NULL;
    ...
    const char* links[2] = { FS_MGR_USERDATA_PARTITION_LNK, NULL }; // 2: links array len
    if (selabel_lookup_best_match(fc_sehandle, &secontext, pathName, links, sb->st_mode) < 0) {
        selinux_log(SELINUX_INFO, "SELinux: not found secontext for %s !\n", pathName);
        return rc;  /* no match, but not an error */
    }
                                             ...
                                             unsigned int flags)
{
    ...
    if (!strncmp(pathname, DATA_APP_EL1_BASE, sizeof(DATA_APP_EL1_BASE) - 1) ||
        !strncmp(pathname, DATA_APP_EL2_BASE, sizeof(DATA_APP_EL2_BASE) - 1) ||
        !strncmp(pathname, DATA_APP_EL3, sizeof(DATA_APP_EL3) - 1) ||
        !strncmp(pathname, DATA_APP_EL4, sizeof(DATA_APP_EL4) - 1) ||
        !strncmp(pathname, DATA_APP_EL1_DATA, sizeof(DATA_APP_EL1_DATA) - 1) ||
        !strncmp(pathname, DATA_APP_EL2_DATA, sizeof(DATA_APP_EL2_DATA) - 1)) {
        goto cleanup;
    }
    if (!recurse) {
    ...
    if (!strcmp(pathname, "/data/sadata_de/samgr") || !strcmp(pathname, "/data/misc_ce/0/dmsdata")) {
        selinux_log(SELINUX_INFO, "fix bath found! path = %s", pathname);
        isSkip = false;
    }
    if (setrestoreconlast) {
        size = getxattr(pathname, RESTORECON_LAST, xattr_value, sizeof fc_digest);
        if (!force && size == sizeof fc_digest && memcmp(fc_digest, xattr_value, sizeof fc_digest) == 0 && isSkip) {
            selinux_log(SELINUX_INFO,
        ...
        if(!isSkip) {
            selinux_log(SELINUX_INFO,
                        "SELinux: Not skipping restorecon_recursive fix(%s)\n",
                        pathname);
        }
    }
...
#ifdef BG_FSCK
    char dataPartition[MAX_USERDATA_DEVICE_NAME_LEN] = { 0 };
    if (strcmp(pathname, "/dev/block") == 0) {
        int ret = readlink(FS_MGR_USERDATA_PARTITION_LNK, dataPartition, sizeof(dataPartition));
        if (ret < 0) {
            selinux_log(SELINUX_ERROR, "%s : readlink for %s failed!\n", __func__, FS_MGR_USERDATA_PARTITION_LNK);
    ...
    while ((ftsent = fts_read(fts)) != NULL) {
        ...
        case FTS_D:
            ...
            if (setrestoreconlast) {
                ...
                if (check_context_match_for_dir(ftsent->fts_path, &new_node, force, error) && isSkip) {
                    selinux_log(SELINUX_INFO,
                ...
                if(!isSkip) {
                    selinux_log(SELINUX_INFO,
                                "SELinux: Not skipping restorecon on fix(%s)\n",
                                ftsent->fts_path);
                }
                if (new_node) {
                    if (!current) {
            ...
            if (strcmp(ftsent->fts_path, DATA_LOG_FILEINDEX) == 0) {
                // Don't label anything below this directory
                fts_set(fts, ftsent, FTS_SKIP);
            }
        default:
#ifdef BG_FSCK
            if (strcmp(dataPartition, ftsent->fts_path) == 0) {
                error |= RestoreconSbUserdata(ftsent->fts_path, ftsent->fts_statp, nochange, verbose);
                break;
            }
#endif
...
int selinux_android_compare_label_hash(const char *path_name)
{
	uint8_t read_digest[SHA1_HASH_SIZE];
	ssize_t read_size = getxattr(path_name, RESTORECON_PARTIAL_MATCH_DIGEST, read_digest, SHA1_HASH_SIZE);

	uint8_t calculated_digest[SHA1_HASH_SIZE];
	bool status = selabel_hash_all_partial_matches(fc_sehandle, path_name, calculated_digest);

	if (status && read_size == SHA1_HASH_SIZE &&
		memcmp(read_digest, calculated_digest, SHA1_HASH_SIZE) == 0) {
		selinux_log(SELINUX_INFO, "SELinux: path=%s selinux hash equal\n", path_name);
		return LABEL_HASH_EQUAL;
	}

	return LABEL_HASH_NOT_EQUAL;
}

int selinux_android_update_label_hash(const char *path_name)
{
	uint8_t calculated_digest[SHA1_HASH_SIZE];
	bool status = selabel_hash_all_partial_matches(fc_sehandle, path_name, calculated_digest);
	if (!status) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s selinux label hash get failed\n", path_name);
		return LABEL_HASH_UPDATE_FAILED;
	}

	if (setxattr(path_name, RESTORECON_PARTIAL_MATCH_DIGEST, calculated_digest, SHA1_HASH_SIZE, 0) < 0) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s setxattr failed\n", path_name);
		return LABEL_HASH_UPDATE_FAILED;
	}

	selinux_log(SELINUX_ERROR, "SELinux: path=%s selinux label hash update success\n", path_name);
	return LABEL_HASH_UPDATE_SUCCESS;
}

int selinux_android_compare_label(const char *path_name, int type)
{
	char *secontext = NULL;
	char *oldsecontext = NULL;
	int rc = LABEL_COMPARE_NOT_EQUAL;

	if (selabel_lookup(fc_sehandle, &secontext, path_name, type) < 0) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s selabel_lookup failed\n", path_name);
		rc = LABEL_LOOKUP_FAIL;
		return rc;
	}

	if (lgetfilecon(path_name, &oldsecontext) < 0) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s lgetfilecon label failed\n", path_name);
		rc = LABEL_GET_INODE_FAIL;
		goto OUT;
	}

	if (!strncmp(oldsecontext, secontext, strlen(oldsecontext))) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s label compare equal\n", path_name);
		rc = LABEL_COMPARE_EQUAL;
	}
...
int selinux_android_process_dir_label(const char *path_name, int type)
{
	if (path_name == NULL) {
		selinux_log(SELINUX_ERROR, "SELinux: %s path_name is Null\n", __FUNCTION__);
		return LABEL_CHECK_FAILED;
	}

	selinux_log(SELINUX_ERROR, "SELinux: path=%s restorecon dir\n", path_name);
	int ret = selinux_android_compare_label(path_name, type);

	if (ret != LABEL_COMPARE_EQUAL) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s selinux label changed\n", path_name);
		return LABEL_CHECK_FAILED;
	}

	ret = selinux_android_update_label_hash(path_name);
	if (ret != LABEL_HASH_UPDATE_SUCCESS) {
		selinux_log(SELINUX_ERROR, "SELinux: path=%s selinux label hash update failed\n", path_name);
		return LABEL_CHECK_FAILED;
	}

	return LABEL_CHECK_SUCCESS;
}

void selinux_android_skip_restorecon_filter_path(const char *src_path, char *filter_path, int flags)
{
	int flag_recursive = SELINUX_ANDROID_RESTORECON_RECURSE | SELINUX_ANDROID_RESTORECON_SKIPCE;
	int dir_check_failed = LABEL_CHECK_SUCCESS;
	char *fts_path[2] = { NULL , NULL };
	FTS *fts;
	FTSENT *ftsent;
	int fts_flags = FTS_NOCHDIR | FTS_PHYSICAL | FTS_XDEV;

	if (src_path == NULL || filter_path == NULL) {
		selinux_log(SELINUX_ERROR, "SELinux: %s src_path or filter_path is Null\n", __FUNCTION__);
		return;
	}
	if (strlen(src_path) != strlen("/data") || strncmp(src_path, "/data", strlen(src_path)) ||
		((flags & flag_recursive) != flag_recursive)) {
		return;
	}
	selinux_log(SELINUX_INFO, "SELinux: restorecon filter path=%s\n", filter_path);
	if (selinux_android_compare_label_hash(filter_path) == LABEL_HASH_EQUAL) {
		return;
	}
	fts_path[0] = filter_path;
	fts = fts_open(fts_path, fts_flags, NULL);
	if (!fts) {
		selinux_log(SELINUX_INFO, "SELinux: fts_open path=%s failed\n", filter_path);
		return;
	}
	while ((ftsent = fts_read(fts)) != NULL) {
		switch (ftsent->fts_info) {
		...
		case FTS_ERR:
			...
			continue;
		case FTS_D:
			if (ftsent->fts_level != FTS_FIRST_LEVEL_SUB_DIR) {
				continue;
			}
			fts_set(fts, ftsent, FTS_SKIP);
			if (selinux_android_process_dir_label(ftsent->fts_path, ftsent->fts_statp->st_mode) == LABEL_CHECK_FAILED) {
				dir_check_failed = LABEL_CHECK_FAILED;
			}
			continue;
		default:
			if ((ftsent->fts_info == FTS_F) && (ftsent->fts_level == FTS_FIRST_LEVEL_SUB_DIR) &&
				(dir_check_failed != LABEL_CHECK_FAILED)) {
	...
	if (dir_check_failed == LABEL_CHECK_SUCCESS) {
		if (selinux_android_restorecon(filter_path, 0) < 0) {
			goto OUT;
		}
		selinux_android_update_label_hash(filter_path);
	}
