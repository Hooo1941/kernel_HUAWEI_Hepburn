...
// rescue mode
static const struct selinux_opt seopts_keystore2_key_plat_rescue[] = {
    { SELABEL_OPT_PATH, "/newsystem/system/etc/selinux/plat_keystore2_key_contexts"},
};

static bool is_rescue_mode()
{
    int rc = 0;
    FILE *cfg;
    char *buf;
    ...
    if (cfg) {
        char *tmp;
        buf = malloc(KCMD_LINE_SIZE);
        if (!buf) {
            fclose(cfg);
            return false;
        }
        if (fgets(buf, KCMD_LINE_SIZE, cfg) &&
        (tmp = strstr(buf, "rescue_mode=true"))) {
            find = true;
        }
        fclose(cfg);
        free(buf);
...
struct selabel_handle* selinux_android_keystore2_key_context_handle(void)
{
    ...
    if (is_rescue_mode()) {
        for (i = 0; i < ARRAY_SIZE(seopts_keystore2_key_plat_rescue); i++) {
            if (access(seopts_keystore2_key_plat_rescue[i].value, R_OK) != -1) {
                seopts_keystore2_key[size++] = seopts_keystore2_key_plat_rescue[i];
                break;
            }
        }
    }
