...
#define XCOLLIE_FLAG_LOG       (1 << 0)        // generate log file
#define XCOLLIE_FLAG_BACKTRACE (1 << 1)        // generate backtrace

typedef int xcollie_t;
typedef xcollie_t (*func_start)(const char *name, int timeout, void (*func)(void *), void *arg, int flag);
typedef void (*func_end)(xcollie_t id);
typedef void (*func_init)(void);

extern func_start fuse_xcollie_start;
extern func_end fuse_xcollie_end;
