...
static const char xcollie_library[] = "libxcollie.so";
static const char xcollie_start_func_name[] = "__xcollie_start_ex";
static const char xcollie_end_func_name[] = "xcollie_end";
static const char xcollie_init_func_name[] = "xcollie_init";

func_start fuse_xcollie_start = NULL;
func_end fuse_xcollie_end = NULL;

void __attribute__((constructor)) init_xcollie_func(void)
{
	void *dlHandler = dlopen(xcollie_library, RTLD_LAZY);
	if (dlHandler == NULL) {
		fuse_log(FUSE_LOG_ERR, "[fuse xcollie_library]: failed to dlopen libxcollie.so\n");
		return;
	}

	fuse_xcollie_start = (func_start)dlsym(dlHandler, xcollie_start_func_name);
	if (fuse_xcollie_start == NULL) {
		fuse_log(FUSE_LOG_ERR, "[fuse xcollie_library]: can not find symbol __xcollie_start_ex\n");
		dlclose(dlHandler);
		return;
	}

	fuse_xcollie_end = (func_end)dlsym(dlHandler, xcollie_end_func_name);
	if (fuse_xcollie_end == NULL) {
		fuse_log(FUSE_LOG_ERR, "[fuse xcollie_library]: can not find symbol xcollie_end\n");
		fuse_xcollie_start = NULL;
		dlclose(dlHandler);
		return;
	}

	func_init fuse_xcollie_init = (func_init)dlsym(dlHandler, xcollie_init_func_name);
	if (fuse_xcollie_init == NULL) {
		fuse_log(FUSE_LOG_ERR, "[fuse xcollie_library]: can not find symbol xcollie_init\n");
		fuse_xcollie_start = NULL;
		fuse_xcollie_end = NULL;
		dlclose(dlHandler);
		return;
	}
	fuse_xcollie_init();

	fuse_log(FUSE_LOG_ERR, "[fuse xcollie_library]: success\n");
}
