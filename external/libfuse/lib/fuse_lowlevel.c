				  ...
				  const struct fuse_buf *buf, struct fuse_chan *ch)
{
	...
	int res;
	xcollie_t id = 0;
	const int fuse_msg_timeout = 60; // fuse msg handle timeout, unit: second
	const int xcollie_flag = XCOLLIE_FLAG_LOG | XCOLLIE_FLAG_BACKTRACE; // disable recovery

	if (fuse_xcollie_start != NULL)
		id = fuse_xcollie_start(__func__, fuse_msg_timeout, NULL, NULL, xcollie_flag);

	if (se->debug) {
		fuse_log(FUSE_LOG_DEBUG, "libfuse xcollie start id = %u,timeout thr = %u, flag = 0x%0x\n",
			id, fuse_msg_timeout, xcollie_flag);
	}
...
out_free:
	...
	if (se->debug)
		fuse_log(FUSE_LOG_DEBUG, "libfuse xcollie end id=%u\n", id);
	return;
