			...
			struct f2fs_node *node, int name)
{
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
		...
		if (c.feature & cpu_to_le32(F2FS_FEATURE_DEDUP)) {
			DISP_u64(inode, i_inner_ino);
			DISP_u32(inode, i_dedup_flags);
			DISP_u32(inode, i_dedup_rsvd);
		}
	}
...
void print_sb_state(struct f2fs_super_block *sb)
{
	...
	if (f & cpu_to_le32(F2FS_FEATURE_DEDUP)) {
		MSG(0, "%s", " dedup");
	}
        if (f & cpu_to_le32(F2FS_FEATURE_DISK_MAGO)) {
                MSG(0, "%s", " diskmago");
	}
	MSG(0, "\n");
...
static void check_compression_dedup_feature(struct f2fs_sb_info *sbi, enum SB_ADDR sb_addr)
{
	bool need_fix = false;

	if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
		if (c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION)) {
			if (!(sbi->raw_super->feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR))) {
				sbi->raw_super->feature |= cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR);
				need_fix = true;
				MSG(0, "Fix feature: extra_attr\n");
			}
			if (!(sbi->raw_super->feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION))) {
				sbi->raw_super->feature |= cpu_to_le32(F2FS_FEATURE_COMPRESSION);
				need_fix = true;
				MSG(0, "Fix feature: compression\n");
			}
		}
		...
		if (c.feature & cpu_to_le32(F2FS_FEATURE_DEDUP)) {
			...
			if (!(sbi->raw_super->feature & cpu_to_le32(F2FS_FEATURE_DEDUP))) {
				sbi->raw_super->feature |= cpu_to_le32(F2FS_FEATURE_DEDUP);
				need_fix = true;
				MSG(0, "Fix feature: dedup\n");
			}
		}
	}
...
int validate_super_block(struct f2fs_sb_info *sbi, enum SB_ADDR sb_addr)
{
	...
	if (!sanity_check_raw_super(sbi->raw_super, sb_addr)) {
		check_compression_dedup_feature(sbi, sb_addr);
		/* write the new device path into sbi->raw_super */
		...
		if (!c.no_kernel_check &&
				cmp_kernel_version(c.sb_version, c.version)) {
			c.auto_fix = 0;
			c.fix_on = 1;
		}
		if (c.fix_on) {
...
int init_sb_info(struct f2fs_sb_info *sbi)
{
	...
	sbi->cur_victim_sec = NULL_SEGNO;
        ...
        c.feature |= (get_sb(feature) & F2FS_FEATURE_DISK_MAGO);  // update diskmago feature by sb
					...
					struct f2fs_summary *sum)
{
	...
	if (c.func == FSCK) {
		if (IS_NODESEG(se->type))
			sum_blk = get_sum_node_block_from_cache(sbi, segno, &type);
		else
			sum_blk = get_sum_data_block_from_cache(sbi, segno, &type);
	}
	if (!sum_blk)
	...
	if (c.func != FSCK &&
	    (type == SEG_TYPE_NODE || type == SEG_TYPE_DATA ||
					type == SEG_TYPE_MAX))
		free(sum_blk);
}
				...
				unsigned int segno, int *ret_type)
{
	...
	*ret_type= SEG_TYPE_MAX;
	...
	/* fsck has already checked curseg in
	 * get_sum_{data|node}_block_from_cache
	 */
	if (c.func == FSCK)
		goto read_ssa_block;
...
read_ssa_block:
	...
	if (c.func == FSCK)
		add_sum_block_to_cache(sbi, segno, *ret_type, sum_blk);
...
static int build_sit_entries(struct f2fs_sb_info *sbi)
{
	...
	unsigned int sit_journal_entries;
...
next_segment:
		...
		if (!(OFFSET_IN_SEG(sbi, *to + 1)) && left)
			*to = START_BLOCK(sbi, segno - 1); /*lint !e647*/
		else
			*to = *to + 1;
	}
	return -1;
...
static int tune_sb_features(struct f2fs_sb_info *sbi)
{
	...
	if (!(sb->feature & cpu_to_le32(F2FS_FEATURE_ENCRYPT)) &&
        ...
        if (!(sb->feature & cpu_to_le32(F2FS_FEATURE_DISK_MAGO)) &&
			(c.feature & cpu_to_le32(F2FS_FEATURE_DISK_MAGO))) {
                sb->feature |= cpu_to_le32(F2FS_FEATURE_DISK_MAGO);
                MSG(0, "Info: Set DiskMago feature\n");
                sb_changed = 1;
	}
