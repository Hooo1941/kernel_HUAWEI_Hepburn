...
void resize_usage()
{
	...
	MSG(0, "  -O feature1[feature2,feature3,...] e.g. \"compression,dedup\"\n");
	MSG(0, "  -V print the version number and exit\n");
	exit(1);
}
...
void sload_usage()
{
	...
	MSG(0, "    -r readonly (to release unused blocks) for compressed files\n");
	MSG(0, "    ------------------------------------------------------\n");
	MSG(0, "  -d debug level [default:0]\n");
	MSG(0, "  -i invert segment allocation direction\n");
	MSG(0, "  -V print the version number and exit\n");
#ifdef HW_F2FS_IMG_OEM
	MSG(0, "  --oem-list=list_path given a config file for files or dirs list\n");
	MSG(0, "  --oem-dir=dir_path just use a given special dir, exclude others\n");
#endif
	exit(1);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog) || !strcmp("fsck.f2fs_s", prog)) {
		...
		const char *option_string = "d:l:st:c:r:O:iV";
...
#ifdef HW_F2FS_IMG_OEM
		struct option long_opt[] = {
			{"oem-list", required_argument, 0, 'F'},
			{"oem-dir", required_argument, 0, 'O'},
			{0, 0, 0, 0}
		};
#else
		struct option long_opt[] = {
...
int main(int argc, char **argv)
{
	...
	gid_t groups[] = { AID_VENDOR_DSM };

	/* set supplementary group AID_VENDOR_DSM for fsck.f2fs process to
	 * access /dev/dsm
	 */
