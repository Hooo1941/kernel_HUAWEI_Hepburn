...
int get_device_info(int i)
{
	...
	struct device_info *dev = c.devices + i;
        if (c.feature & cpu_to_le32(F2FS_FEATURE_DISK_MAGO)) {
            if (dev->sector_size != c.devices->sector_size) {
                MSG(0, "\tInfo: current dev sector size = %u, force set to %u for DiskMago.\n",
                    dev->sector_size, c.devices->sector_size);
                dev->sector_size = c.devices->sector_size;
            }
        }
