...
static void mkfs_usage()
{
	...
	MSG(0, "  -r # of reserved segments for dynamic resize\n");
...
{
	static const char *option_string = "qa:c:C:d:e:E:g:hil:mo:O:R:s:S:z:t:fw:r:V";
	...
	while ((option = getopt_long(argc,argv,option_string,long_opts,NULL)) != EOF) {
		...
		case 'r':
			if (c.ndevs >= MAX_DEVICES) {
				MSG(0, "Error: Reserve too many devices\n");
				mkfs_usage();
			}
			c.devices[c.ndevs].path = RESERVED_DEV;
			c.devices[c.ndevs].total_segments = atoi(optarg);
			c.devices[c.ndevs].reserved = 1;
			c.ndevs++;
			break;
		case 'V':
			show_version("mkfs.hmfs");
			exit(0);
		case 'C':
			token = strtok(optarg, ":");
