...
int get_device_index(__u64 blk_addr)
{
	int i;
...
int dev_write(void *buf, __u64 offset, size_t len, int sync_before,
		int sync_after, int stream_id)
{
	int fd;
	...
	if (sync_before) {
		...
		set_write_stream_id(stream_id, offset >> F2FS_BLKSIZE_BITS);
	}
#endif
...
{
	return dev_write(buf, blk_addr << F2FS_BLKSIZE_BITS, F2FS_BLKSIZE, 0, 1, 0);
}

int dev_write_block_no_sync(void *buf, __u64 blk_addr)
{
	return dev_write(buf, blk_addr << F2FS_BLKSIZE_BITS, F2FS_BLKSIZE, 0, 0, 0);
}
...
{
	blk_addr = get_dm_source_addr(blk_addr);
