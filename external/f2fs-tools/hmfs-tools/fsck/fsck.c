		...
		struct dnode_of_data *dn)
{
	...
	/* readahead DIR data blocks */
	if (ftype == F2FS_FT_DIR) {
		for (idx = 0; idx < ADDRS_PER_BLOCK(inode); idx++) {
...
static int f2fs_check_dirent_position(const struct f2fs_dir_entry *dentry,
						const unsigned char *printable_name,
...
int fsck_chk_meta(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < TOTAL_SEGS(sbi); i++) {
		...
		if (se->valid_blocks != 0)
			sit_valid_segs++;
		else if (IS_CUR_SEGNO(sbi, i) || IS_DM_SOURCE_SEGNO(sbi, i)) {
			/* curseg has not been written back to device */
			MSG(1, "\tInfo: curseg %u and data move souce segment "
					"is counted in valid segs\n", i);
			sit_valid_segs++;
		}
		if (IS_NODESEG(se->type))
...
void fix_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	*((__le32 *)((unsigned char *)cp + CP_CHKSUM_OFFSET)) = cpu_to_le32(crc);
	...
	if (flags & CP_APPEND_NAT_FLAG) {
		...
		block_t start_blk =  __next_cp_addr(sbi) +
...
void fsck_free(struct f2fs_sb_info *sbi)
{
	...
	if (fsck->nat_area_dirty_bitmap)
		free(fsck->nat_area_dirty_bitmap);
