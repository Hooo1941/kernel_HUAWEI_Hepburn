			...
			nid_t dir_ino)
{
	...
	for (i = 0; i < entries; i++) {
		...
		ret = asprintf(&dentries[i].path, "%s%s",
			...
			ret = asprintf(&dentries[i].path, "%s/%s",
					c.target_special_dir, namelist[i]->d_name);
		}
#endif
		ASSERT(ret > 0);
...
#ifdef HW_F2FS_IMG_OEM
			set_path_passed_for_file(dentries[i].path);
#endif
