...
struct free_segmap_info {
	...
	u8 *free_dm_segmap;	/* free data move segment bitmap */
};
...
static inline void get_nat_bitmap(struct f2fs_sb_info *sbi, void *dst_addr)
{
	struct f2fs_nm_info *nm_i = NM_I(sbi);

	memcpy(dst_addr, nm_i->nat_bitmap, nm_i->bitmap_size);
}
...
static inline block_t __next_cp_addr(struct f2fs_sb_info *sbi)
{
	block_t start_addr = le32_to_cpu(F2FS_RAW_SUPER(sbi)->cp_blkaddr);

	if (sbi->cur_cp == CP_VER_FIRST)
		start_addr += sbi->blocks_per_seg;
	return start_addr;
}
...
{
	sbi->cur_cp = (sbi->cur_cp == CP_VER_FIRST) ? CP_VER_SECOND : CP_VER_FIRST;
}
...
static bool IS_IN_CURSEC_OFFSET(struct f2fs_sb_info *sbi, u32 addr)
{
	int i;
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		/*
		 * If current section of datamove stream is empty, it means
		 * that last call of datamove just ended at the end of last
		 * section, and current section hasn't been allocated to
		 * datamove stream.
		 */
		if (IS_DM_SEG(i) && IS_FIRST_DATA_BLOCK_IN_SEC(sbi, start))
			continue;

		if (addr >= start && addr < end) {
			ASSERT_MSG("block addr:%x is in cursec:%x offset\n",
							addr, cur_secno);
			return true;
		}
	}
...
static inline bool IS_VALID_BLK_ADDR(struct f2fs_sb_info *sbi, u32 addr)
{
	...
	if (c.func == FSCK && c.fsck_stat >= CHECKING &&
			IS_IN_CURSEC_OFFSET(sbi, addr))
		return false;
	/* next block offset will be checked at the end of fsck. */
	return true;
}

static inline bool IS_IN_MAIN_AREA(struct f2fs_sb_info *sbi, block_t addr)
{
	if (addr >= le64_to_cpu(F2FS_RAW_SUPER(sbi)->block_count) ||
				addr < SM_I(sbi)->main_blkaddr) {
		DBG(1, "block addr [0x%x] is not in main area\n", addr);
		return false;
	}
	return true;
}

static inline bool IS_DM_SOURCE_SEGNO(struct f2fs_sb_info *sbi, u32 segno)
{
	struct free_segmap_info *free_i = FREE_I(sbi);

	if (test_bit_le(segno, free_i->free_dm_segmap))
		return true;
	else
		return false;
}
