...
static int f2fs_prepare_super_block(void)
{
	...
	if (normal_total_zones <= avail_zones) {
...
static int f2fs_write_check_point_pack(void)
{
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		set_cp(free_segment_count, f2fs_get_usable_segments(sb) - 2 - rsvd_dev_segs);
		set_cp(user_block_count, ((get_cp(free_segment_count) + 2 -
			get_cp(overprov_segment_count)) * c.blks_per_seg));
	} else {
		set_cp(free_segment_count, f2fs_get_usable_segments(sb) - 6 - rsvd_dev_segs);
...
static int f2fs_write_root_inode(void)
{
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_DEDUP)) {
		raw_node->i.i_inner_ino = 0;
		raw_node->i.i_dedup_flags = 0;
		raw_node->i.i_dedup_rsvd = 0;
	}
