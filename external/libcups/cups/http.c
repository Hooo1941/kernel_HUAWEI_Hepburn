	       ...
	       int    *cancel)		/* I - Pointer to "cancel" variable */
{
  ...
  if (!http)
  {
    ...
    LOGE("_cupsSetError, IPP_STATUS_ERROR_INTERNAL");
    return (-1);
  }
  ...
  if (http->tls)
  {
    ...
    LOGD("2httpReconnect2: Shutting down SSL/TLS...");
    _httpTLSStop(http);
  }
#endif /* HAVE_SSL */
  ...
  */
  ...
  if (http->fd >= 0)
  {
    ...
    LOGD("2httpReconnect2: Closing socket");
  ...
  if ((addr = httpAddrConnect2(http->addrlist, &(http->fd), msec, cancel)) == NULL)
  {
    ...
    */
    ...
    DEBUG_printf(("1httpReconnect2: httpAddrConnect failed: %s",
                  strerror(http->error)));
    LOGE("1httpReconnect2: httpAddrConnect failed: %s",
                  strerror(http->error));
  ...
  */
  ...
  if (!http->tls)
  {
    ...
    */
    ...
    LOGD("8http_tls_upgrade: Server does not support HTTP upgrade!");

    _cupsSetError(IPP_STATUS_ERROR_CUPS_PKI, _("Encryption is not supported."), 1);
    LOGE("_cupsSetError, IPP_STATUS_ERROR_CUPS_PKI");
    httpAddrClose(NULL, http->fd);
