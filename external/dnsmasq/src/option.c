...
static struct {
    ...
    {LOPT_REUSEADDR, 0, NULL, gettext_noop("Reuse the specified DHCP port."), NULL},
...
void read_opts(int argc, char** argv, char* compile_opts) {
    ...
    while (1) {
        ...
        if (option == LOPT_TEST)
            ...
            nest++;
        } else if (option == LOPT_REUSEADDR) {
            daemon->reuse_addr = OPT_REUSEADDR;
        } else {
#ifdef HAVE_GETOPT_LONG
