    ...
    switch (mess_type) {
        ...
        case DHCPREQUEST:
            ...
            if (message) {
                ...
                log_packet("DHCPACK", &mess->yiaddr, emac, emac_len, iface_name, NULL,
                           mess->xid);
        ...
        case DHCPINFORM:
           ...
           get one from MAC address/client-d */
            ...
            log_packet("DHCPACK", &mess->ciaddr, emac, emac_len, iface_name, NULL, mess->xid);
...
void change_mac(char* mac_input, size_t len)
{
    if (mac_input == NULL) {
        return ;
    }
...
static void log_packet(char* type, void* addr, unsigned char* ext_mac, int mac_len, char* interface,
    ...
    static const int ADDR_MAX_SIZE = sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255");
    ...
    change_mac(mac_buf, strlen(mac_buf));

    char str[ADDR_MAX_SIZE];
    if (sizeof(ip_buf) < strlen(inet_ntop(AF_INET, &addr_msg, str, sizeof(str))) ||
        sprintf(ip_buf, "%s", inet_ntop(AF_INET, &addr_msg, str, sizeof(str))) < 0) {
