...
static int mkfs_parse_options_cfg(int argc, char *argv[])
{
...
#ifdef  EROFS_UTILS121
        s64 timestamp;
	while((opt = getopt_long(argc, argv, "m:j:b:g:i:I:e:o:T:l:L:a:S:C:B:d:h:N:z::fwJsctvUxE",
				 long_options, NULL)) != -1) {
#else
	while((opt = getopt_long(argc, argv, "d:x:z:E:T:U:",
		...
		case 'T':
			...
			if (timestamp != -1)
 				cfg.c_unix_timestamp = timestamp;
		...
		case 'h':
		    cfg.c_shared_xattr_threshold = strtoull(optarg, &endptr, 10);
			break;
		...
		case 'l':
		    cfg.c_img_len = strtoull(optarg, &endptr, 10);
			break;

		case 'm':
		    cfg.c_multi_blks_thr = strtoull(optarg, &endptr, 10);
			break;
	...
	if (optind < argc) {
#if defined(ANDROID)
		cfg.target_out_path = strdup(argv[optind++]);
#else
		erofs_err("can't set android target_out_path - built without android support\n");
		return -EINVAL;
#endif
	}
#endif
