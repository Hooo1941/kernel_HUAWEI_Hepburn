...
static int erofs_xattr_add(struct list_head *ixattrs, struct xattr_item *item)
{
	...
	if ((int)(item->count) == cfg.c_inline_xattr_tolerance + 1) {
...
#endif
#if PATCH_ENABLED
int erofs_prepare_xattr_ibody(struct erofs_inode *inode, struct erofs_inode *org_inode)
#else
...
#if PATCH_ENABLED
	if (org_inode)
		ret = read_xattrs_from_file(org_inode->i_srcpath, org_inode->i_mode, ixattrs);
	else
