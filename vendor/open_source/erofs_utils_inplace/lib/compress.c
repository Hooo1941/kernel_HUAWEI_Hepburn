...
nocompression:
			...
			erofs_dbg("Writing %u compressed data to block %u",
				  count, ctx->blkaddr);
#if PATCH_ENABLED
			if (is_write && erofs_sb_has_lz4_0padding())
...
#endif
{
	...
	memset(compressmeta, 0, sizeof(Z_EROFS_LEGACY_MAP_HEADER_SIZE));
#if PATCH_ENABLED
	if (org_inode) {
...
#if PATCH_ENABLED
	inode->nblocks = compressed_blocks;
	inode->startaddr = blkaddr;
	if (!org_inode)
		ret = erofs_bh_balloon(bh,
			blknr_to_addr(compressed_blocks));
#else
