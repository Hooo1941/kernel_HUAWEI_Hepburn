...
#ifdef HDR_VIVID_VIDEO
    CUVVConfiguration cuvv_config = hdr_get_cuva_from_metadata(s, mov, track);
    if (cuvv_config.cuva_version_map > 0 && track->par->codec_id == AV_CODEC_ID_HEVC) {
        memset(compressor_name, 0, strlen(compressor_name));
        const int CUVA_COMPRESSOR_NAME_LEN = 14;
        memcpy(compressor_name, "CUVA HDR Video", CUVA_COMPRESSOR_NAME_LEN);
    }
#endif
...
#ifdef HDR_VIVID_VIDEO
    if (cuvv_config.cuva_version_map > 0 && track->par->codec_id == AV_CODEC_ID_HEVC) {
        hdr_write_cuvv_tag(pb, &cuvv_config);
    }
#endif
    if (track->mode == MODE_MP4 &&
            ((ret = mov_write_btrt_tag(pb, track)) < 0))
