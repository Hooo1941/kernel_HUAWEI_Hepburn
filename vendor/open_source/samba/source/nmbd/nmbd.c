...
{
	LOGD("[nmbd] Got SIGTERM: going down...");
...
static void process(void)
{
	...
	while( True ) {
		...
		/*
		* listen_for_packets may block 10 seconds, so t will not be correct
		* move it here can be correct
		*/
		time_t t = time(NULL);
...
static void *main_nmbd_read_socket_thread(void *arg)
{
	...
	if (NULL == arg) {
		DEBUG(0, ("main_nmbd_read_socket_thread wrong input\n"));
		exit(1);
	}
	sockfd = *(int *)arg;
	while(1) {
		...
		buffer_recv = (char *)SMB_MALLOC(buf_size);
                if (buffer_recv == NULL) {
                    DEBUG(0, ("SMB_MALLOC fail\n")); /* 0 indicates log printing level */
                    exit(1); /* 1 means abnormal exit program */
                }
		...
		if(*((uint16*)(buffer_recv)) == NMBD_CLOSE_COMMAND) {
 ...
 int main(int argc, const char *argv[])
{
	...
	LOGD(" Netbios nameserver version %s started. ", SAMBA_VERSION_STRING);
...
#ifdef ADD_UNIX_SOCKET
	main_nmbd_sockfd = open_unix_socket();
	send_unix_socket(NMBD_START_SUCCESS, main_nmbd_sockfd, NULL);
	if (pthread_create(&read_tid, NULL, main_nmbd_read_socket_thread, (void *)&main_nmbd_sockfd) < 0) {
